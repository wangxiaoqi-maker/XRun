...
## 十五、AI 自愈能力扩展（参考货拉拉实践）

> 参考文章：[货拉拉UI自动化新范式：从人工维护到AI自愈](https://mp.weixin.qq.com/s/EKjCIHOKVosPacKuyGqT4w)（货拉拉技术 · 质量保障部）

### 15.1 背景与可借鉴点

货拉拉在移动端 UI 自动化中通过 **「感知-诊断-决策-修复」** 的 AI 自愈闭环，将脚本通过率从约 80% 提升至 90%+，并节省约 40% 脚本维护人力。与本项目的对比如下：

| 维度 | 当前 XRun | 货拉拉实践 | 可优化方向 |
|------|-----------|------------|------------|
| 失败处理 | 步骤级 retry + on_fail 策略 | 失败后 AI 诊断 → 生成修复动作 → 步骤级重试 | 引入执行时自愈钩子 |
| 弹窗 | 用例内 ai_act「关闭弹窗」、try-finally | 平台级弹窗特征建模，VLM 识别类型并决策 | 弹窗智能处理引擎 |
| 元素定位 | 元素库 midscene_locator + 回退到 elementName | 控件画像（类型+关系上下文+结构）提升鲁棒性 | 扩展元素库为「控件画像」 |
| 定位器失效 | 依赖人工更新元素库 | 多模态基线比对 + 选择器重构/优化，自动更新 | 文本/元素自愈（二期） |

以下为可落地的设计扩展，在不改变现有用例格式的前提下增强执行稳定性与可维护性。

### 15.2 控件画像（元素库增强）

**目标**：让元素不仅存「定位器」，还存「数字身份证」，便于自愈时做变更感知与定位器重构。

**现状**：`kb_page_element` 已有 `description`、`midscene_locator`、`position_area`、`position_in_container`、`relative_positions` 等，已具备部分「画像」信息。

**建议扩展**（与现有字段兼容，可逐步落地）：

- **控件类型**：已有 `element_type`；可补充语义角色（如 `button_primary`、`link_navigation`），供自愈时区分「橙色确认按钮」与任意「确认」文本。
- **关系上下文**：用 `description` / `midscene_locator` 承载「隐私政策协议底部的同意按钮」等页面语义；可选字段 `context_label`（如「协议底部」「弹窗内」）便于检索与比对。
- **结构关系**：可选字段 `dom_path_snapshot`、`parent_container_desc`，用于抗布局微调；当前 `position_in_container`、`relative_positions` 可视为轻量版。

**自愈时的使用**：执行失败时，将当前步骤的控件画像（类型+上下文+结构）与当前页面多模态数据（截图/OCR/DOM）一并交给大模型，用于判断是「弹窗打断」「文案微调」还是「元素/布局重构」，并生成修复动作（见下）。

### 15.3 执行时自愈钩子（感知-诊断-决策-修复）

**目标**：步骤执行失败时，不立即按 on_fail 结束，而是先走一层「自愈流程」：诊断 → 若为可自愈问题则执行修复动作 → 再对当前步骤重试。
...
步骤执行失败
    ...
    ▼
┌─────────────────────────────────────────────────────────────────┐
│ 自愈开关：case/suite 级配置 self_heal: true（默认可关闭）          │
└─────────────────────────────────────────────────────────────────┘
    │ 若开启
    ▼
┌─────────────────────────────────────────────────────────────────┐
│ 1. 采集多模态上下文                                               │
│    - 当前截图、步骤定义、element_ref、错误信息、设备/会话状态      │
│    - 若已实现基线：当前 OCR、DOM 快照                             │
└─────────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────────┐
│ 2. 自愈引擎诊断（可分层，与货拉拉五步诊断对齐）                    │
│    a. 设备/会话健康 → 不可自愈则直接失败                          │
│    b. 弹窗类干扰 → 走弹窗智能处理，输出 Action（如关闭/同意）      │
│    c. 元素/文案/布局变更 → 走变更感知，输出定位器修复或选择器重构  │
└─────────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────────┐
│ 3. 执行修复动作（弹窗关闭 / 动态替换定位器后重试）                 │
└─────────────────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────────────────┐
│ 4. 仅对当前步骤重试（保留业务状态，不重跑全用例）                  │
│    成功 → 可选：将新定位器回写元素库或用例映射，形成闭环          │
└─────────────────────────────────────────────────────────────────┘
```

**与现有能力的关系**：

- 现有 **retry / retry_interval**：仍保留，作为「同一逻辑步骤」的简单重试；自愈在「首次失败后」插入，相当于「先诊断再决定是否用新动作重试」。
- 现有 **try-catch / on_fail**：自愈若仍失败，再按 on_fail（stop/continue/skip_rest）处理。

**配置示例**（扩展 suite/case YAML）：

```yaml
# 套件或用例级
self_heal:
  enabled: true
  max_heal_attempts: 2          # 每步骤最多自愈尝试次数
  popup_handling: true          # 是否启用弹窗智能处理
  element_heal: true            # 是否启用元素/文案自愈（依赖基线时可开）
```

### 15.4 弹窗智能处理引擎

**目标**：将「关闭弹窗」从用例内手写步骤升级为平台能力，由执行引擎在适当时机自动识别并处理。

**思路**（对齐货拉拉）：

- **弹窗特征建模**：在平台侧维护轻量「弹窗知识库」：类型（权限申请、系统通知、隐私合规、厂商定制等）、典型关键词、典型按钮语义（同意/拒绝/稍后/确定）。
- **识别与决策**：步骤失败或步骤前（可配置）截屏 + OCR/ accessibility 树，送入 VLM 或规则+模型混合，判断是否被弹窗打断；若是，则输出结构化动作（如「点击同意」「点击关闭」）。
- **执行**：由执行引擎插入一次或多次「弹窗处理」动作（调用 Midscene ai_act 或 tap），再重试原步骤。

**与现有用法兼容**：用例中已有的 `ai_act: 关闭所有弹窗`、try-finally 中关闭弹窗仍可保留；平台弹窗引擎可在「自愈钩子」或「步骤前检查」中优先执行，减少用例内重复编写。

### 15.5 文本/元素自愈（二期）

**目标**：当定位器因文案微调、ID 重构、布局重构失效时，通过「基线 + 变更感知」自动生成新定位器并步骤级重试。

**依赖**：

- **基线**：用例成功执行时采集并存储「步骤级」多模态基线（截图、OCR 文本、DOM 或可访问性树片段），并与控件画像关联。
- **变更感知**：失败时采集当前截图/OCR/DOM，与基线 + 控件画像一起交给大模型，做语义/位置/结构三维比对，输出变更类型（文案优化、元素重构、布局调整）及建议的新定位器。
- **修复**：选择器优化（微调条件）或选择器重构（生成新 midscene_locator），动态替换后仅重试当前步骤；可选将新定位器回写元素库。

**与现有元素库的关系**：Element Resolver 当前是「编译时」解析 element_ref → locator；自愈场景下需支持「运行时」注入修复后的 locator（或临时替换 element_ref 的解析结果），再重试当前步骤。

### 15.6 实施优先级建议

| 优先级 | 能力 | 说明 |
|--------|------|------|
| P0 | 执行时自愈钩子 + 配置项 | 先打通「失败 → 诊断 → 修复动作 → 步骤重试」的框架，诊断可先做简单规则（如仅弹窗） |
| P1 | 弹窗智能处理 | 平台弹窗知识库 + 截图/OCR 识别 + 决策，显著减少因弹窗导致的失败 |
| P2 | 控件画像增强 | 在现有元素库上增加上下文/结构字段，供自愈与后续基线比对使用 |
| P3 | 基线采集 + 文本/元素自愈 | 依赖执行历史存储与多模态比对，成本较高，可作为二期 |

以上扩展均保持与现有 Midscene 执行引擎、YAML 用例格式、元素库引用方式兼容，便于分阶段落地。
...
- [货拉拉UI自动化新范式：从人工维护到AI自愈](https://mp.weixin.qq.com/s/EKjCIHOKVosPacKuyGqT4w)（TesterHome / 货拉拉技术）