...
class Executor(object):
    ...
    def my_assert(self, asserts: List, json_format: bool) -> [str, bool]:
        ...
        for index, item in enumerate(asserts):  # 遍历断言
            try:
                ...
                status, err = self.ops(
                    item.get("assert_type"), expected, actually
                )  # 判断预期结果和实际结果
            ...
            for param in out_params:
                if param.get("extract_obj") == "response_json":
                    try:
                        ...
                        logger.info(
                            "出参类型为response_json，提取成功，提取表达式为：{}，提取结果为：{}".format(
                                param.get("extract_exp"), value
                            )
                        )
                        self.append(
                            f"出参类型为response_json，提取成功，提取表达式为：{param.get('extract_exp')}，提取结果为：{value}"
                        )
                    ...
                    try:
                        value = str(data)
                        logger.info(
                            "出参类型为response_text，提取成功，提取表达式为：{}，提取结果为：{}".format(
                                param.get("extract_exp"), value
                            )
                        )
                        self.append(
                            f"出参类型为response_text，提取成功，提取表达式为：{param.get('extract_exp')}，提取结果为：{value}"
                        )
                        out_params_list.append({param.get("param_name"): value})
                        setattr(Data, param.get("param_name"), value)
                    except Exception as e:
                    ...
                    try:
                        value = JSONPathParser().header_parse(
                            data, param.get("extract_exp")
                        )
                        logger.info(
                            "出参类型为response_headers，提取成功，提取表达式为：{}，提取结果为：{}".format(
                                param.get("extract_exp"), value
                            )
                        )
                        self.append(
                            f"出参类型为response_headers，提取成功，提取表达式为：{param.get('extract_exp')}，提取结果为：{value}"
                        )
                        out_params_list.append({param.get("param_name"): value})
                        setattr(Data, param.get("param_name"), value)
                    except Exception as e:
    ...
    def add_cases(self, cases, envs, suite_id):
        ...
        for index, _case in enumerate(cases):
            ...
            def test(selfs, case=_case, env=envs):
                ...
                data = Request(url, body=bodys).request(
                    method=method, body_type=body_type, headers=headers, body=bodys
                )
                Interfaces.objects.filter(id=case.id).update(
                ...
                if assert_list:
                    actual = JSONPathParser().parse_assert(
                        data, params_list.assert_list
                    )
                    ...
                    for mes in message:
                        if message[mes].get("status"):
                            ...
                            Interfaces.objects.filter(id=case.id).update(
                                desc=self.logger.join()
                            )
                            self.logger.log.clear()
                            TestSuit.objects.filter(id=suite_id).update(
                                status=str(result),
                                state="已完成",
    ...
    def create_report(request, message, suite_id):
        ...
        body = {
            ...
            "total_count": result.success_count
    ...
    def replace_single_interface_params(self, cases):
        ...
        cases["request_headers"] = json.dumps(
            self.parse_list_and_replacre(request_headers)
        )
...
if __name__ == "__main__":
    exp = 1.05
    act = 1.05
    s = Executor().ops("int_greater_than_or_equal", exp, act)
    print(s)
